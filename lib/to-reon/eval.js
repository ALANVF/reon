...
var Macro = /*#__PURE__*/function () {
  function Macro(params, locals, body) {
    ...
    this.locals = function () {
      ...
      for (i = 0, len = locals.length; i < len; i++) {
        name = locals[i];
        results.push([name, _token5.Value.NONE]);
      }
  ...
  _createClass(Macro, [{
    ...
    value: function call(env, tokens) {
      ...
      try {
        tmpEnv = env.newInner(args.concat(this.locals));
...
mustGetWord = function mustGetWord(env, word) {
  ...
  if ((value = env.get(word)) != null) {
    ...
    throw new Error("Undefined word `".concat(word, "`!"));
  }
};
