...
$macro = function $macro(_, _ref3) {
  ...
  var j, len, locals, paramK, paramV, params;
  expectToken(paramsK, _token["default"].block);
  expectToken(bodyK, _token["default"].block);
  params = [];
  locals = [];

  for (j = 0, len = paramsV.length; j < len; j++) {
    var _paramsV$j = _slicedToArray(paramsV[j], 2);
    ...
    switch (paramK) {
      case _token["default"].word:
        params.push([_eval.Param.val, paramV]);
        break;

      case _token["default"].getWord:
        params.push([_eval.Param.get, paramV]);
        break;

      case _token["default"].litWord:
        params.push([_eval.Param.lit, paramV]);
        break;

      case _token["default"].setWord:
        locals.push(paramV);
        break;

      default:
        unexpectedToken(paramK);
    }
  }

  return new _eval.Macro(params, locals, _toConsumableArray(bodyV));
};
...
$continue = function $continue(_, arg) {
  var arg;
  arg;
  throw new _eval.ControlFlow.Continue();
};
/* Conversion */
